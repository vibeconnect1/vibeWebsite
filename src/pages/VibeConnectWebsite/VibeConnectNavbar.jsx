...
const VibeConnectNavbar = () => {
  ...
  return (
    <div>
    <nav className={`fixed top-0 z-50 w-full bg-white/2 transition-transform duration-300 ${showNavbar ? 'translate-y-0' : '-translate-y-full'}`}>
    ...
    >
      ...
      <div className="md:flex gap-8  hidden">
        <div className="relative">
          <button className="font-bold flex text-white items-center" onClick={toggleDropdown}>
            Apps
            <span className="ml-1"><FaChevronDown size={12} /></span>
          </button>
        </div>
        <p className="font-bold text-white z-10">About us</p>
       
        <p className="font-bold text-white z-10"><Link to="/contact">Contact</Link></p>
      </div>
      
      <div className="md:flex hidden z-20">
        <button onClick={openSignInModal} className="font-bold text-white px-4 py-2 rounded-md mr-4 z-10">Sign in</button>
